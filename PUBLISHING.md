...
7. Confirm the duplicate fixture's workflow is skipped, then confirm scenario A
...
12. If successful runs approach the configured `max-ai-credits` limit, raise the
   limit before the event and recompile the workflow.
13. Replace the representative files in `fallback-outputs/` with output captured
   during the rehearsal.
14. Keep one facilitator-owned repository and Codespace ready for a live demo.
